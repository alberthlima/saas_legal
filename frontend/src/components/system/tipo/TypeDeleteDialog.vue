...
const emit = defineEmits(["update:isDialogVisible", "deleteType"]);
    ...
    const response = await $api("/type/" + props.typeSelected.id, {
      ...
      onResponse({ response }) {
        if (response.status === 200) {
          ...
          emit("deleteType", props.typeSelected.id);
        } else {
          toast.showError(
            response._data.message || "Error al eliminar el tipo de documento",
            "Error",
          );
        }
      },
    ...
    <VCard class="pa-sm-8 pa-4">
      ...
      <VCardText class="pt-5">
        <div class="text-center pb-6">
          ...
          <p class="text-h6 mb-2">{{ props.typeSelected.name }}</p>
