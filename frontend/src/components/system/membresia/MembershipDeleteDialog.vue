...
const emit = defineEmits(["update:isDialogVisible", "deleteMembership"]);
    ...
    const response = await $api("/membership/" + props.membershipSelected.id, {
      ...
      onResponse({ response }) {
        if (response.status === 200) {
          toast.showSuccess(
            response._data.message || "Membresía eliminada correctamente",
            "Éxito",
          );
          emit("update:isDialogVisible", false);
          emit("deleteMembership", props.membershipSelected.id);
        } else {
          toast.showError(
            response._data.message || "Error al eliminar la membresía",
            "Error",
          );
        }
      },
      ...
      <VCardText class="pt-5">
        <div class="text-center pb-6">
          <VIcon
            ...
            class="mb-4"
          />
          <h4 class="text-h4 font-weight-bold mb-3">¿Eliminar Membresía?</h4>
          <p class="text-h6 mb-2">{{ props.membershipSelected.name }}</p>
