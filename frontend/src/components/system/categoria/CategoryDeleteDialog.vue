...
const emit = defineEmits(["update:isDialogVisible", "deleteCategory"]);
    ...
    const response = await $api("/category/" + props.categorySelected.id, {
      ...
      onResponse({ response }) {
        if (response.status === 200) {
          toast.showSuccess(
            response._data.message || "Categoria eliminada correctamente",
            "Éxito",
          );
          emit("update:isDialogVisible", false);
          emit("deleteCategory", props.categorySelected.id);
        } else {
          toast.showError(
            response._data.message || "Error al eliminar la categoria",
            "Error",
          );
        }
      },
      ...
      <VCardText class="pt-5">
        <div class="text-center pb-6">
          <VIcon
            ...
            class="mb-4"
          />
          <h4 class="text-h4 font-weight-bold mb-3">¿Eliminar Categoria?</h4>
          <p class="text-h6 mb-2">{{ props.categorySelected.name }}</p>
