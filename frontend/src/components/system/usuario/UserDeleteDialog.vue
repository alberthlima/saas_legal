    ...
    const response = await $api("/user/" + props.userSelected.id, {
      ...
      onResponse({ response }) {
        if (response.status === 200) {
          toast.showSuccess(response._data.message || 'Usuario eliminado correctamente', 'Éxito');
          emit('update:isDialogVisible', false);
          emit('deleteUser', props.userSelected.id);
        } else {
          toast.showError(response._data.message || 'Error al eliminar el usuario', 'Error');
        }
      }
    })
  } catch (error) {
    ...
    <VCard class="pa-sm-8 pa-4">
      ...
      <VCardText class="pt-5">
        <div class="text-center pb-6">
          <VIcon icon="ri-delete-bin-line" size="64" color="error" class="mb-4"/>
          <h4 class="text-h4 font-weight-bold mb-3">¿Eliminar Usuario?</h4>
          <p class="text-h6 mb-2">{{ props.userSelected.full_name }}</p>
        ...
        <VRow class="justify-center mt-4">
          <VCol cols="12" class="d-flex gap-3 justify-center">
            <VBtn color="error" class="px-8" :loading="isLoading" :disabled="isLoading" @click="deleteUser">
